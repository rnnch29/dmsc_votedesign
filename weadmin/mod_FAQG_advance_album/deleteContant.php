...
for($i=1;$i<=$_REQUEST['TotalCheckBoxID'];$i++) {
 $myVar=$_REQUEST['CheckBoxID'.$i];
	...
	$permissionID=$myVar;
			...
			$sql = "SELECT  ".$mod_tb_root."_htmlfilename,".$mod_tb_root."_pic,".$mod_tb_root."_htmlfilenameen,".$mod_tb_root."_picen  FROM ".$mod_tb_root." WHERE  ".$mod_tb_root."_id='".$permissionID."' ";
			...
			// ######################### Delete  In Folder File ###############################
			$sql="SELECT ".$mod_tb_root_album."_id,".$mod_tb_root_album."_filename, " .$mod_tb_root_album."_filenameen FROM ".$mod_tb_root_album." WHERE ".$mod_tb_root_album."_contantid ='".$permissionID."'";
			...
			if($number_row>=1){
				...
				$deletefilenameen =$row_file[2];
					...
					// DEL ENG FILES
					if(file_exists($mod_path_album."/".$deletefilenameen)) {
						@unlink($mod_path_album."/".$deletefilenameen);
					}	

					if(file_exists($mod_path_album."/reB_".$deletefilenameen)) {
						@unlink($mod_path_album."/reB_".$deletefilenameen);
					}	

					if(file_exists($mod_path_album."/reO_".$deletefilenameen)) {
						@unlink($mod_path_album."/reO_".$deletefilenameen);
					}	
				}
			}
